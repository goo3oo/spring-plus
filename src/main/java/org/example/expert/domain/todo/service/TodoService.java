...
public class TodoService {
    ...
    public Page<TodoResponse> getTodos(TodoGetRequest todoGetRequest) {
        Pageable pageable = PageRequest.of(
                todoGetRequest.getPage() - 1, todoGetRequest.getSize()
        );

        Page<Todo> todos = todoRepository.findAllByOrderByModifiedAtDesc(
                pageable,
                todoGetRequest.getStartDate(),
                todoGetRequest.getEndDate(),
                todoGetRequest.getWeather()
        );
    ...
    public Page<TodoSearchResponse> searchTodos(TodoSearchRequest todoSearchRequest) {
        Pageable pageable = PageRequest.of(
                todoSearchRequest.getPage() - 1, todoSearchRequest.getSize()
        );

        return todoRepository.searchTodos(
                todoSearchRequest.getKeyword(),
                todoSearchRequest.getStartDate(),
                todoSearchRequest.getEndDate(),
